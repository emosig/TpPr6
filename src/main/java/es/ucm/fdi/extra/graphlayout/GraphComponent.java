...
public class GraphComponent extends JComponent {
	...
	/**
	 * An inner class that represent a location of a node. Fields cX and cY 
	 * are the center of the node, and fields tX and tY are the location where
	 * the label of the node is drawn. This is calculated for each node in 
	 * the method {@code calculateNodeCoordinates()}
	 */
	private class Point {
		int cX;
		int cY;
	...
	/**
	 * width and height of the window when it was last resized. When change 
	 * we recalculate the location of nodes to scale the graph, etc.
	 */
		...
		for (Edge e : _graph.getEdges()) {
			...
			for (Dot d : e.getDots()) {
				...
				Color dotColor = Math.random() > 0.5 ? Color.MAGENTA :
					Color.ORANGE;
				drawCircleOnALine(g, p1.cX, p1.cY, p2.cX, p2.cY, 
						e.getLength(), d.getLocation(), diam, dotColor,
						d.getId());
			}
		}
	}
	...
	/**
	 * put the objects in a circle, for each one store the center coordinate 
	 * and a coordinate for a corresponding text.
	 */
	private void calculateNodeCoordinates() {

		int r = Math.min(_lastHeight, _lastWidth) / 2 - _nodeRadius - 50; 
		// 50 for text
	 ...
	 * (virtual) length of the line is virtualLength, the circles is drawn 
	 * at location virtualLocation (0..virtualLength). The diameter is 'diam'
	 */
	private void drawCircleOnALine(Graphics g, int x1, int y1, int x2, int y2, 
		...
		double actualLocation = lineActualLength * 
				((double) virtualLocation) / ((double) virtualLength) + 15;
		...
		// draw the point
		g.setColor(c);
		g.drawOval(x1 + xDir * ((int) x) - diam / 2, y1 + yDir * 
				((int) y) - diam / 2, diam, diam);
